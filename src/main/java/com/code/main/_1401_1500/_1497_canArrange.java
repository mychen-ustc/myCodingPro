...
class Solution_1497 {
    public boolean canArrange(int[] arr, int k) {
        ...
        // 将数组遍历一次，以余数为key，统计每个余数对应数字的个数
        for (int num : arr) {
            int remainder = (num % k + k) % k;  // 保证余数范围在0~k-1
            map.put(remainder, map.getOrDefault(remainder, 0) + 1);
        }
        // 遍历余数，余数为0时，个数要是偶数。余数大于0时，假设为m， 则 m 和 k -m 的值必须相同
        for (int key : map.keySet()) {
            if (key == 0) {
                if (map.get(key).intValue() % 2 != 0) {    // 余数为0时，value必须是偶数
                    return false;
                }
            } else {
...
//        int[] arr = {1, 2, 3, 4, 5, 6};
//        int k = 10;
        Solution_1497 solution = new Solution_1497();
        boolean ans = solution.canArrange(arr, k);
