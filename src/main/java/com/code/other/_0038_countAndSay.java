 ...
 * //
 * // 例如，数字字符串 "3322251" 的描述如下图 _0038_countandsay.jpg：
 * //
 * // 示例 1：
 * // 输入：n = 1
 * // 输出："1"
...
class Solution_0038 {
    public String countAndSay(int n) {
        String ans = "1";     // pre记录前一个字符串
        StringBuffer buffer = new StringBuffer();
        for (int i = 1; i < n; i++) {  // 逐个生成描述字符串
            buffer = new StringBuffer();
            char num = ans.charAt(0);
            int cnt = 1;
            for (int j = 1; j < ans.length(); j++) {    // 遍历pre
                char ch = ans.charAt(j);
                if (ch == num) {
                    cnt++;
                } else {
                    buffer.append(cnt).append(num); // 追加当前数字的描述
                    num = ch;
                    cnt = 1;
                }
            }
            // 处理最后一组数字描述
            buffer.append(cnt).append(num);
            ans = buffer.toString();
        }
        return ans;
    }
...
public class _0038_countAndSay {
    public static void main(String[] args) {
        Solution_0038 solution = new Solution_0038();
        for (int i = 1; i < 30; i++) {
